				...
				<view v-for="(item, index) in courseList" :key="index+'item'">
					<view v-for="(subitem, subindex) in item.zcd" :key="subindex">
						...
						<view v-if='subitem===weekIndex+1'>
							<!-- 点击查看详细情况，style则包含其在课程表中的位置 -->
							<view class="kcb-item radius" @click="showDetail(index)"
								:style="'margin-left:'+((item.xqj-1)*12.8+0.4)+'vw;margin-right:0.4vw;margin-top:'+((item.jcs-1)*120+5)+'rpx;height:'+(item.cxjs*120-5)+'rpx;background-color:'+item.bg+';width:12vw;z-index: 9;'">
								<!-- 课程名称以及教学教师 -->
