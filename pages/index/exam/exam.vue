	...
	<view>
		...
		<view class="animation-slide-bottom margin list-container" v-if="examList.length">
			<view class="list-head bg-blue-1">
				<view class=""> 课程名/地点 </view>
				<view class=""> 时间 </view>
				<view class=""> 剩余时间 </view>
			</view>
			<view class="list-item text-black" v-for="(item, index) in examList" :key="index"
		...
		<cu-empty :type="emptyType">
			<view slot="text" class="">
				...
				<button v-if="emptyType == 'search'" class="cu-btn bg-orange-1 round shadow-blur" @click="getExam">
					查询
				</button>
			</view>
		</cu-empty>
