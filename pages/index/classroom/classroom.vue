  ...
  <view>
    ...
    <view class="cu-list menu sm-border">
      <view class="cu-item">
        <view class="content">
          <button
            class="cu-btn bg-orange-1 shadow"
            @click="modalName = 'WeekModal'"
          >
            第{{ thisWeek }}周
          </button>
        </view>
      ...
      <view class="cu-item">
        <view class="content">
          ...
          <button
            class="cu-btn bg-orange-1 shadow"
            @click="modalName = 'LocalModal'"
          >
            {{ curBuilding }}
          </button>
        </view>
        <!-- 通过修改modalName来唤起相应的对话框-->
        <view class="action">
          <button
            class="cu-btn bg-orange-1 shadow"
            @click="modalName = 'JcModal'"
          >
            第{{ jcText }}大节
          </button>
        </view>
    ...
    <view class="padding flex flex-direction">
      <!-- 查询教室的变化 -->
      <button class="cu-btn bg-orange-1 round shadow-blur lg" @click="getClass">
        查询
      </button>
    </view>
    <view
      class="cu-modal bottom-modal"
      :class="modalName == 'WeekModal' ? 'show' : ''"
    >
      <view class="cu-dialog">
        ...
        <view class="grid col-4 padding-sm bg-gray">
          <view v-for="(item, index) in 20" class="padding-xs" :key="index">
            <button
              class="cu-btn blue block"
              :class="index + 1 == thisWeek ? 'bg-blue' : 'line-blue'"
              @click="thisWeek = index + 1"
            >
              第{{ index + 1 }}周
            </button>
          </view>
      ...
      <view class="cu-dialog">
        ...
        <view class="grid col-4 padding-sm bg-gray">
          <view v-for="(item, index) in 7" class="padding-xs" :key="index">
            <button
              class="cu-btn blue block"
              :class="index == thisDay ? 'bg-blue' : 'line-blue'"
              @click="thisDay = index"
            >
              星期{{ title[index] }}
            </button>
    ...
    <view
      class="cu-modal bottom-modal"
      :class="modalName == 'LocalModal' ? 'show' : ''"
    >
      <view class="cu-dialog">
        <view class="cu-bar bg-white">
          <view class="action text-blue" @click="modalName = null">单选</view>
        ...
        <view class="grid col-3 padding-sm bg-gray">
          <view
            v-for="(item, index) in scBuildings"
            class="padding-xs"
            :key="index"
          >
            <button
              class="cu-btn blue block"
              :class="index + 1 == thisLocal ? 'bg-blue' : 'line-blue'"
              @click="
                thisLocal = item.id;
                curBuilding = item.name;
              "
            >
              {{ item.name }}
            </button>
    ...
    <view class="grid col-3 text-center text-bold padding">
      <view
        v-for="(item, index) in emptyClassList"
        :key="index"
        class="solids padding"
      >
        {{ item }}
      </view>
    </view>
...
export default {
  ...
  methods: {
    ...
    // todo 比较少
    getClass: function () {
      this.emptyClassList = ["7B201", "7B302", "7B212"];
