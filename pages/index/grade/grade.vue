	...
	<view>
		...
		<view class="animation-slide-bottom margin list-container" v-if="gradeList.length">
			<view class="list-head bg-blue-1">
				<view class=""> 课程名（点击查看详情分数） </view>
				<view class=""> 选中/总评 </view>
			</view>
			<view class="list-item text-black" v-for="(item, index) in gradeList" :key="index"
				...
				<view class="list-subitem margin-top-sm">
					...
					<view class="flex-sub text-center"> 绩点：{{ item.jd }} </view>
				...
				for (let i = 0; i < this.gradeList.length; i++) {
					if (this.choFlag[i]) {
						...
						switch (this.gradeList[i].cj) {
							...
							default:
								vjq +=
									parseFloat(this.gradeList[i].cj) *
									parseFloat(this.gradeList[i].xf);
						}

						vjd +=
							parseFloat(this.gradeList[i].jd) * parseFloat(this.gradeList[i].xf);
					}
				}
